...
def plot_cost_history(cost_history):
    ...
    :return:NULL
    :用于分析代价函数随着模型训练的收敛情况，以及可能的收敛速度和稳定性
    """
    plt.figure('代价函数')
...
if __name__ == "__main__":
    # 读取训练数据
    training_data = pd.read_csv('data_2.csv', header=None)
    # 获取特征和标签
    X_train = training_data.iloc[:, :4].values.T#训练集特征矩阵
    Y_train = training_data.iloc[:, 4:].values.T#训练集的标签矩阵
    Y_train = Y_train.astype('uint8')#将数据类型转换为无符号8位整数
    # 训练模型
    start_time = datetime.datetime.now()
    #掩藏层单元数量10，输入层特征数量4，输出层特征数量3，迭代次数10000
    ...
    # 对模型进行测试
    test_data = pd.read_csv('data_2.csv', header=None)
